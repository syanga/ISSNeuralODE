...
function solve_ss(
    ...
    state_dim,
    batchsize;
    u0_guess = zeros(state_dim, batchsize),
    abstol = 1e-10,
)
    ...
    # return x0
    ssprob = NonlinearProblem{false}(
        (x, p) -> vec(model(vcat(reshape(x, state_dim, batchsize), u0), p)),
        vec(u0_guess),
        p_ode,
    )
    return reshape(
        Array(solve(ssprob, NewtonRaphson(), tol = abstol)),
        state_dim,
        batchsize,
    )
end
