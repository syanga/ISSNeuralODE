...
struct LinearFeedthrough{F} <: Feedthrough
    ...
    function LinearFeedthrough(input_dim::Int, output_dim::Int; initW=Flux.glorot_uniform)
        ...
        new{typeof(initial_params)}(input_dim, output_dim, i-1, index_dict, initial_params)
    end
end
...
struct ResistiveFeedthrough{F} <: Feedthrough
    ...
    index_dict::Dict{String, UnitRange{Int}}
    initial_params::F
    function ResistiveFeedthrough(input_dim::Int, output_dim::Int; initW=Flux.glorot_uniform)
        # compute indices
        i = 1
        index_dict = Dict{String, UnitRange{Int}}()
...
function unpack_params(f::ResistiveFeedthrough, p)
    D = hcat(diagm(p[f.index_dict["D"]].^2), zeros(f.output_dim, max(0, f.input_dim-f.output_dim)))
    return D
end
