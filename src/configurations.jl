...
function run_config(c::TrainConfig; runs=1)
    ...
    println("Number of BLAS threads: $(ccall((:openblas_get_num_threads64_, Base.libblas_name), Cint, ())
    ...
    for _=1:runs
        ...
        trainer = DynamicsTrainer(
                    ode, link, mod, feed, solver, sensealg, Array{dtype,1}(t), Array{dtype,3}(u), Array{dtype,3}(y);  
                    savepath=savepath, 
                    dtype=dtype, 
                    validation_split=validation_split, test_split=test_split,
                    abstol=abstol, reltol=reltol, dt=dt, adaptive=adaptive,
                    initial_condition=initial_condition, 
                    loss=loss, regularization=regularization, regularization_coef=regularization_coef, optim=optim, wdecay=wdecay, sched=sched,
        ...
        train(trainer, num_epochs; 
            ...
            patience=patience, logevery=logevery, verbose=verbose)
    end
end
