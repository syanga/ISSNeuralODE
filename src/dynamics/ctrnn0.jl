...
struct CTRNN0Elman{F,FU,F2} <: CTRNN0
    ...
    σ_u::FU    
    initial_params::F2
    function CTRNN0Elman(state_dim::Integer, input_dim::Integer; σ=tanh, initW=Flux.glorot_uniform, initb=Flux.zeros, σ_u=identity)
        # compute indices
        i = 1
        index_dict = Dict{String, UnitRange{Int}}()
        index_dict["logτ"] = i:i
        ...
        new{typeof(σ),typeof(σ_u),typeof(initial_params)}(state_dim, input_dim, i-1, index_dict, σ, σ_u, initial_params)
    end
end
...
struct CTRNN0Canonical{F,FU,F2} <: CTRNN0
    ...
    σ_u::FU
    initial_params::F2
    function CTRNN0Canonical(state_dim::Integer, input_dim::Integer; σ=tanh, initW=Flux.glorot_uniform, initb=Flux.zeros, σ_u=identity)
        # compute indices
        i = 1
        index_dict = Dict{String, UnitRange{Int}}()
        index_dict["logτ"] = i:i
...
struct CTRNN0Zero{F,FU,F2} <: CTRNN0
    ...
    σ_u::FU
    initial_params::F2
    function CTRNN0Zero(state_dim::Integer, input_dim::Integer; σ=tanh, initW=Flux.glorot_uniform, σ_u=identity)
        # compute indices
        i = 1
        index_dict = Dict{String, UnitRange{Int}}()
        index_dict["logτ"] = i:i
