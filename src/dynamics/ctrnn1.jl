...
struct CTRNN1Nominal{F,FU,F2} <: CTRNN1
    ...
    σ_u::FU
    initial_params::F2
    function CTRNN1Nominal(state_dim::Int, hidden_dim::Int, input_dim::Int; σ=relu, initW=Flux.glorot_uniform, initb=Flux.zeros, σ_u=identity)
        # compute indices
        i = 1
        index_dict = Dict{String, UnitRange{Int}}()
        index_dict["logτ"] = i:i
        ...
        new{typeof(σ),typeof(σ_u),typeof(initial_params)}(state_dim, hidden_dim, input_dim, i-1, index_dict, σ, σ_u, initial_params)
    end
end
...
struct CTRNN1Canonical{F,FU,F2} <: CTRNN1
    ...
    σ_u::FU
    initial_params::F2
    function CTRNN1Canonical(state_dim::Int, hidden_dim::Int, input_dim::Int; σ=relu, initW=Flux.glorot_uniform, initb=Flux.zeros, σ_u=identity)
        # compute indices
        i = 1
        index_dict = Dict{String, UnitRange{Int}}()
        index_dict["logτ"] = i:i
...
struct CTRNN1Zero{F,FU,F2} <: CTRNN1
    ...
    σ_u::FU
    initial_params::F2
    function CTRNN1Zero(state_dim::Int, hidden_dim::Int, input_dim::Int; σ=tanh, initW=Flux.glorot_uniform, initb=Flux.zeros, σ_u=identity)
        # compute indices
        i = 1
        index_dict = Dict{String, UnitRange{Int}}()
        index_dict["logτ"] = i:i
